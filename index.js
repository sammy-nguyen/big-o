...
// ANSWER: constant--NO, cannot rewrite
...
function squareEveryDigit(number) {
  ...
  const sepString = stringNumber.split("");

  let arr = [];
  for (let i = 0; i < sepString.length; i++) {
    let squaredNumber = parseInt(sepString[i]);
    squaredNumber = Math.pow(squaredNumber, 2);
    arr.push(squaredNumber.toString());
  }
